...
public class UserService {
    ...
    public Mono<User> getUserDetailsById() {
        return MappingUtils.mapSecurityContextToUid()
                .flatMap(id -> userDao.findById(id)
                        .switchIfEmpty(Mono.just(new User().setUserID(id))));
    }

    public Mono<User> addRestaurantToFav(String restaurantId) {
        return MappingUtils.mapSecurityContextToUid()
                .flatMap(uid -> restaurantDao.findById(restaurantId)
                        .flatMap(restaurant -> userDao.findById(uid)
