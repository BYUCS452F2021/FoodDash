...
public class UserControllerTests {
    ...
    public void addFav() {
        user = webTestClient
                .put().uri(builder -> builder.path("/user/favorite/{id}").build(restaurant.getId()))
                .exchange()
                .expectStatus().isOk()
    ...
    public void deleteFav() {
        user = webTestClient
                .delete().uri(builder -> builder.path("/user/favorite/{id}").build(restaurant.getId()))
                .exchange()
                .expectStatus().isOk()
                .returnResult(User.class).getResponseBody().blockFirst();

        assertNotNull(user);
        assertTrue(user.getFavoriteRestaurants() == null || user.getFavoriteRestaurants().isEmpty(), "Favorite list should be empty");
    }
}
